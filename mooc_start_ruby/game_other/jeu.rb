class Personne
	...
	def info
		if @en_vie== true
			puts @nom + " a " + points_de_vie + " points de vie !" # - Renvoie le nom et les points de vie si la personne est en vie
		else
			puts @nom + " est vaincu !" # - Renvoie le nom et "vaincu" si la personne a été vaincue
		end
	end

	def attaque(personne)
		@nom = personne
		personne.points_de_vie = personne.points_de_vie - 25# - Fait subir des dégats à la personne passée en paramètre
		puts personne +" a perdu 25 points de vie."# - Affiche ce qu'il s'est passé
		puts personne +" a maintenant " + personne.points_de_vie + " point de vies !"
	end

	def subit_attaque(degats_recus)

		@points_de_vie = @points_de_vie - degats_recus # - Réduit les points de vie en fonction des dégats reçus
		puts degats_recus +" dégats reçus !" # - Affiche ce qu'il s'est passé
		if @points_de_vie > 0 # - Détermine si la personne est toujours en_vie ou non
			@en_vie = true
		else @envie = false
		end
...
class Joueur < Personne
	...
	def soin
		@point_de_vie = @point_de_vie + 15# - Gagner de la vie
		puts @nom +" a gagné 15 points de vie !"# - Affiche ce qu'il s'est passé
	end
