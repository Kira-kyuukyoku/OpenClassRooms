class Personne
  ...
  def initialize(nom)
    # Attention a bien utiliser le @ devant les variables sinon cela ne fonctionne pas.
    @nom = nom
    @points_de_vie = 100
    @en_vie = true
  end
  ...
  def attaque(personne)
    ...
    #personne.points_de_vie = points_de_vie-55
...
class Joueur < Personne
  ...
  def degats
    ...
    # - Affiche ce qu'il s'est passé
    if @degats_bonus != 0
      impact = (rand*10+1).round + @degats_bonus
    else
      impact = (rand*10+1).round
    end
    puts @nom + " perd #{impact}hp."

    # Attention la dernière valeur doit toujours être le return sinon erreur. :(
    @points_de_vie -= impact
  end
  ...
  def soin
    ...
    if points_de_vie < 100
      @points_de_vie += (rand*10+1).round
      puts nom + " récupére de la vie."
    else
      puts nom + " tente de récupérer de la vie mais échoue."
    end
  end
  ...
  def ameliorer_degats
    ...
    # - Affiche ce qu'il s'est passé
    @degats_bonus += (rand*10+1).round
    puts @nom + " augmente son attaque."
  end
end
...
class Jeu
  ...
  def self.est_fini(joueur, monde)
    ...
    # Si le joueur ou tous les ennemis sont morts
